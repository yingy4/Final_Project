...
class StudentBuildLike protected() extends CommonBuild {
  ...
  lazy val submitSetting = submit := {
    ...
    val connectMsg =
      s"""|Attempting to submit """.stripMargin
    ...
    def reportCourseraResponse(response: HttpResponse[String]): Unit = {
      ...
      code match {
        // case Success, responds with 2xx HTTP status code
        case cde if cde >= 200 && cde < 300 =>
          val successfulSubmitMsg =
            s"""|Assignment submitted successfully!
                """.stripMargin
          s.log.info(successfulSubmitMsg)

        // case Failure, responds with 4xx HTTP status code (client-side failure)
    ...
    postSubmission(json) match {
      ...
      case Failure(e) =>
        val failedConnectMsg =
          s"""|There was something wrong while attempting to connect .
